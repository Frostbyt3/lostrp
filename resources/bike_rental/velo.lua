	...
	menu = {
		...
		["Velo-Classique"] = { 
			...
			buttons = { 
				...
				{name = "Bicycle", costs = 20, description = {}, model = "cruiser"},			
			}
		},
		["Velo-Sport"] = { 
			title = "Bikes - Sports", 
			name = "Bikes - Sports",
			buttons = { 
				{name = "Bike sport", costs = 20, description = {}, model = "fixter"},
				{name = "Bike course 1", costs = 20, description = {}, model = "tribike"},
				{name = "Bike course 2", costs = 20, description = {}, model = "tribike2"},
				{name = "Bike course 3", costs = 20, description = {}, model = "tribike3"},
			}
		},
	}
}
-- entering = circle /// -- inside = the ped  /// outside = purchase the vehicle //// the fourth positions = angle
			...
			while #velshop_blips > 0 do
				...
				for i,b in ipairs(velshop_blips) do
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and velshop.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 0.4001 then
						...
						drawTxt('Press ~g~ENTER~s~ to access the bicycles',0,1,0.5,0.8,0.6,255,255,255,255)
			...
			for i,button in pairs(menu.buttons) do
				if i >= velshop.menu.from and i <= velshop.menu.to then
					...
					if button.costs ~= nil then
						if velshop.currentmenu == "Bikes - Classic" or velshop.currentmenu == "Bikes - Sports" then
...
function ButtonSelected(button)
	...
	if this == "main" then
		if btn == "Classic Bikes" then
			OpenMenu('Bikes - Classic')
		elseif btn == "Sports Bikes" then
			OpenMenu('Bikes - Sports')
		end
		
	elseif this == "Bikes - Classic" or this == "Bikes - Sports" then
...
function Back()
	...
	if velshop.currentmenu == "main" then
		CloseCreatorvelo()
	elseif velshop.currentmenu == "Bikes - Classic" or velshop.currentmenu == "Bikes - Sports" then
		if DoesEntityExist(fakevelo.velo) then
			Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(fakevelo.velo))
		end
		fakevelo = {model = '', velo = nil}
