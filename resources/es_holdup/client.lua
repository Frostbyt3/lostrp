...
AddEventHandler('es_holdup:toofarlocal', function(robb)
	holdingup = false
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Robbery Cancelled. You will not receive anything!")
	robbingName = ""
	secondsRemaining = 0
	incircle = false
end)
...
AddEventHandler('es_holdup:robberycomplete', function(robb)
	holdingup = false
	TriggerEvent('chatMessage', 'SYSTEM', {255, 0, 0}, "Robbery Complete! You stole $" .. stores[store].reward)
	store = ""
	secondsRemaining = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(stores)do
		...
		AddTextComponentString("Warehousing")
...
Citizen.CreateThread(function()
	while true do
				...
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							...
							DisplayHelpText("Press ~INPUT_CONTEXT~ to rob ~b~" .. v.nameofstore .. "~s~. ~w~The police will be notified.")
						end
						incircle = true
		...
		if holdingup then
			...
			drawTxt(0.66, 1.44, 1.0,1.0,0.4, "Robbery: ~r~" .. secondsRemaining .. "~w~ seconds remaining", 255, 255, 255, 255)
