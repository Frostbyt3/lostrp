    ...
    if not checkOutfit(source, outfit.id) then
        TriggerEvent('es:getPlayerFromId', source, function(user)
            if tonumber(user.money) >= tonumber(outfit.price) and tonumber(user.money) > 0 then
                ...
                TriggerClientEvent('clotheshop:notifs', user.source, "You paid ~y~"..outfit.price.." for this article of clothing."  )
                TriggerClientEvent('clotheshop:notifs', user.source, "Item has been saved to your wardrobe."  )
            else
                TriggerClientEvent('clotheshop:notifs', user.source, "You do not have enough cash on hand."  )
            end
        end)
    end
end)
