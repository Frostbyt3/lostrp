	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		if (tonumber(user.money) >= tonumber(price)) then
			...
			if (tonumber(max_number_weapons) > tonumber(nb_weapon)) then
				...
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Roberto", false, "Enjoy!")
			else
				TriggerClientEvent('ToManyWeapons',source)
				TriggerClientEvent("es_freeroam:notify", source, "CHAR_MP_ROBERTO", 1, "Roberto", false, "You have reached the maximum amount of weapons (max: "..max_number_weapons..")\n")
			end
		else
