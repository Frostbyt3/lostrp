	...
	menu = {
		...
		["main"] = {
			...
			buttons = {
				...
				{title = "Assault rifles", name = "AssaultRifles", description = ""},
		...
		["Pistolets"] = {
			...
			buttons = {
				...
				{title = "Flare Gun", name = "FlareGun", costs = 7500, description = {}, model = "WEAPON_FlareGun"},
			}
		},
		["MachineGuns"] = {
			title = "Machine guns",
			...
			while #weashop_blips > 0 do
				...
				for i,b in ipairs(weashop_blips) do
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and weashop.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 2 then
						...
						drawTxt('Press ~g~ENTRER~s~ to purchase ~b~weapons',0,1,0.5,0.8,0.6,255,255,255,255)
						currentlocation = b
						inrange = true
					end
				end
