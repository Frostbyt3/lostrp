...
function MenuGarage()
    ...
    Menu.addButton("Spawn vehicle","ListeVehicule",nil)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(garage_location[1],garage_location[2],garage_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			drawTxt('Press ~g~E~s~ to open garage menu',0,1,0.5,0.8,0.6,255,255,255,255)		
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(vente_location[1],vente_location[2],vente_location[3],GetEntityCoords(LocalPed())) < 5 and IsPedInAnyVehicle(LocalPed(), true) == false then
			drawTxt('Press ~g~E~s~ to sell the vehicle at 50% of the purchase price',0,1,0.5,0.8,0.6,255,255,255,255)		
...
AddEventHandler('garages:SpawnVehicle', function(vehicle, plate, state, primarycolor, secondarycolor, pearlescentcolor, wheelcolor)
	...
	Citizen.CreateThread(function()			
		...
		if DoesEntityExist(caisseo) then
			drawNotification("Spawn area is full") 
		else
...
function Chat(t)
	TriggerEvent("chatMessage", 'GARAGE', { 0, 255, 255}, "" .. tostring(t))
end
...
AddEventHandler('garages:StoreVehicle', function(vehicle, plate)
	...
	Citizen.CreateThread(function()		
		...
		if DoesEntityExist(caissei) then	
			if (string.lower(string.gsub(plate, " ", "")) ~= string.lower(string.gsub(platecaissei, " ", ""))) then					
				drawNotification("This is not your vehicle")
			else
				Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(caissei))
				drawNotification("Vehicle returned")
