...
AddEventHandler('chasse:drawSellViande', function (qte)
		...
		if qte > 0 then
			...
			local salaire = math.random(20, 30)
			TriggerServerEvent('mission:completed', salaire)
			ClearPrints()
			SetTextEntry_2("STRING")
			AddTextComponentString("~g~You sold fresh meat~s~!")
       ...
       playerPed = GetPlayerPed(-1)
		...
		if pos then
				...
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -1492.2, 4971.5, 64.2, true) <= 8 ) then
					...
					drawTxt("Press ~g~E~s~ to Start or Stop hunting",0,1,0.5,0.8,0.6,255,255,255,255)
						...
						if isinjob==false then
							...
							if not DoesEntityExist(vehiculeDetected) then
								...
								Citizen.CreateThread(function()
									...
									AddTextComponentString("Vehicle spawn area is full.")
						...
						if (missionRunning == true and entitySpawned == true) then	
							...
							if (GetDistanceBetweenCoords( GetEntityCoords(GetPlayerPed(-1)),entityCoords[proie].x , entityCoords[proie].y , entityCoords[proie].z, true ) < 1 and entityAlive == false) then
								if IsPedInAnyVehicle(GetPlayerPed(-1), true) == false then
									...
									ClearPrints()
									SetTextEntry_2("STRING")
									AddTextComponentString("~g~You have picked up meat")
									...
									drawTxt("Get our of the vehicle to pick up meat",0,1,0.5,0.8,0.6,255,255,255,255)
								end
