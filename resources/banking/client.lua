    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {0, 255, 0}, "^0Press 'E' to manage your money.");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You cannot manage your money from your car.");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You cannot use an ATM in your car.");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You may only deposit money at a bank.");
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You cannot manage your money while in your car.");
    else
      TriggerServerEvent("bank:withdraw", tonumber(amount))
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You are not at a bank or ATM.");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1The specified player is not online.");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1No players are within range.");
  end
end)
